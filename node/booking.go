...
// 建立付款-異步的事務節點
func CreatePaymentAsync() orchestrator.AsyncNode {
	return func(topic orchestrator.Topic, data []byte, next orchestrator.Next, rollback orchestrator.Rollback) {
...
func CreateOrderSync() orchestrator.SyncNode {
	return func(requestID string, ctx *ctx.Context) error {
		...
		if !isExist {
			return eris.Wrap(pkgerror.ErrInternalError,"Can not get request in first node")
		}
...
// 建立付款單-同步的事務節點
func CreatePaymentSync() orchestrator.SyncNode {
	return func(requestID string, ctx *ctx.Context) error {

...
* Orchestrator's responsibility is integrating distributed transaction in microservice, all upstream service just needs to maintain gRPC endpoint.
* The flows supported by Orchestrator including: Sync Flow, Async Flow(Using MQ), and throttling flow which locks request, and queue them.
* Just needs to implement framework structure, it will be registered and scaled easily.
* We can change driver of Message Queue, Now supports RabbitMQ and NATS.
* A flow is combined by multiple Nodes, Node can be re-use by other flow.
...
````sh
    ...
    # start up related service on local
    cd deployment/local
    docker-compose up -d
    cd ../..
    go run main.go
...
* A Flow is combined by multiple nodes, node can be re-use.
...
* If something wrong happened, just call rollback node.
* The important thing in Message is traceID (or x-request-id) of request chain.
* If rollback triggered, Orchestrator invoke cancel func to each service, service will cancel data by traceID
...
* Data is delivery in JSON format via MQ. orchestrator(and it's replicas) push message to itself. then send to service via gRPC.
* Response directly when it just received request. Meanwhile, start precess in background (MQ)
...
* Using MQ to control Queue and Concurrency counts, fix that upstream overloading during high concurrency.
* It locks request, and do task in background. if task finished, response to client.
...
Register in singleton, it will be invoked in concurrency
```go
...
In concurrency(or a request), we can get registered flow from singleton. just use run() to execute flow.
